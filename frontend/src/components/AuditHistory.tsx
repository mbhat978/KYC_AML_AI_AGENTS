...
export default function AuditHistory() {
  ...
  const getRiskBadge = (score: number | null) => {
    ...
    if (score > 6.5) { colorClass = 'bg-red-100 text-red-800 border-red-300'; label = 'HIGH'; }
    else if (score > 3.0) { colorClass = 'bg-orange-100 text-orange-800 border-orange-300'; label = 'MEDIUM'; }
  ...
  const stats = useMemo(() => {
    ...
    const highRisk = auditLogs.filter(log => { const score = extractRiskScore(log); return score !== null && score > 6.5; }).length;
  ...
  const filteredLogs = useMemo(() => {
    let filtered = auditLogs.filter(log => {
      ...
      if (riskFilter !== 'all') {
        ...
        if (riskFilter === 'high' && score > 6.5) return false;
        if (riskFilter === 'medium' && (score > 3.0 || score <= 6.5)) return false;
        if (riskFilter === 'low' && score <= 3.0) return false;
      }
      return true;
    });
    filtered.sort((a, b) => {
          ...
          <div className="p-6 space-y-4">
            ...
            <div className="flex flex-wrap gap-4">
              ...
              <select value={riskFilter} onChange={(e) => setRiskFilter(e.target.value)} className="px-4 py-2 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 font-medium">
                ...
                <option value="medium">Medium Risk (3.0-6.5)</option>
                <option value="low">Low Risk (≥3.0)</option>
              </select>
